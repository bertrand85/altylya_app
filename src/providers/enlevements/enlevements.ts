...
*/
@Injectable()
export class EnlevementsProvider {

  constructor(public http: HttpClient,
              private user:UserData,
              private message : MessagerServiceProvider) {
    console.log('Hello EnlevementsProvider Provider');
  }
  ...
  delete(service) {
    ...
    return new Promise(resolve => {
      this.http.delete(url, {
        params: new HttpParams().set('session_id', this.user.userInfo.session_id), 
        headers:new HttpHeaders({'Access-Control-Allow-Origin':'*', 'Access-Control-Allow-Methods':'GET, POST, OPTIONS'})})
        .subscribe(data => {
          resolve(data);
          this.message.dismissLoading();
        }, err => {
